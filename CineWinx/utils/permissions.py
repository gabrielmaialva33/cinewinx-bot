...
async def authorised(func: callable, sub_func_2: callable, client: app, message: Message, *args: tuple, **kwargs: dict):
...
async def unauthorised(
        message: Message, permission: str, sub_func_2: callable, bot_lacking_permission=False
):
    chat_id = message.chat.id
    if bot_lacking_permission:
        text = (
                "Eu não tenho a permissão necessária para realizar esta ação."
                + f"\n<b>Permissão:</b> __{permission}__"
        )
    else:
        text = (
...
def admins_only(permission: str):
    def sub_func(func: callable):
        @wraps(func)
        async def sub_func_2(client, message: Message, *args, **kwargs):
            chat_id = message.chat.id
            ...
            if permission not in bot_perms:
                return await unauthorised(
                    message, permission, sub_func_2, bot_lacking_permission=True
                )
            ...
            if not message.from_user:
                ...
                return await unauthorised(message, permission, sub_func_2)
            ...
            if user_id not in SUDOERS and permission not in permissions:
                return await unauthorised(message, permission, sub_func_2)
            return await authorised(func, sub_func_2, client, message, *args, **kwargs)
