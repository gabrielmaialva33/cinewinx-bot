...
class AnimiZeYAPI:
    ...
    async def search_anime(self, query: str, page_token: Union[str, None] = None):
        return await self.request(
            "/0:search",
            "POST",
            {"q": query, "page_token": page_token or None, "page_index": 0},
        )
    ...
    async def download(self, file_name: str, link: str):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}{link}", headers=self.download_headers
            ) as response:
    ...
    async def get_animezey_search_anime(
        self, query: str, page_token: Union[str, None] = None
    ):
