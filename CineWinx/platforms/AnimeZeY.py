...
class AnimiZeYAPI:
    ...
    async def search_movie(self, query: str, page_token: Optional[str] = None) -> Optional[SearchMovieResponse]:
        response: Optional[SearchMovieResponse] = await self.request('/1:search', 'POST', {
            'q': query,
            'page_token': page_token,
            'page_index': 0,
        })
    ...
    async def download(self, file_name: str, link: str) -> str:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f'{self.base_url}{link}', headers=self.session_headers) as response:
                with open(file_path, 'wb') as f:
                    f.write(await response.read())
        return file_path

    async def movie_foder(self) -> Union[Dict[str, Any], str, None]:
        return await self.request('/1:/Filmes/', 'POST', {
            'id': '',
            'type': 'folder',
            'password': '',
            'page_token': '',
